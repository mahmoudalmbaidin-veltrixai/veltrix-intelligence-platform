# Independent WebKit stress. Does not modify the frozen application tree.
...
Write-Host 'WebKit collapsed-nav 10x'
& node .\node_modules\@playwright\test\cli.js test tests/e2e/login-navigation.spec.ts --grep "collapsed navigation stays discoverable" --project=webkit-desktop --repeat-each=10
$navExit = $LASTEXITCODE

Write-Host 'WebKit timezone CERT-P2-001 10x'
& node .\node_modules\@playwright\test\cli.js test tests/e2e/release-p2-certification.spec.ts --grep "CERT-P2-001" --project=webkit-desktop --repeat-each=10
$tzExit = $LASTEXITCODE

Write-Host "NAV_EXIT=$navExit TZ_EXIT=$tzExit"
if ($navExit -ne 0) { exit $navExit }
exit $tzExit
