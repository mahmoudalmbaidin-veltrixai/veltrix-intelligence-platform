...
interface SchemaRow { table: string; columns: number; rows: number; type: string }
...
interface PreviewRow { id: number; customer: string; amount: number; status: string; placed: string }
...
const previewColumns: Column<PreviewRow>[] = [
  ...
  { key: 'amount', label: 'amount', align: 'right', cell: (r) => formatNumber(r.amount, { style: 'currency', currency: 'USD', decimals: 0 }) },
...
interface Dependency { name: string; type: string; icon: string }
...
interface AuditItem { id: string; event: string; actor: string; ip: string; when: string }
    ...
    <template v-else>
      ...
      <section v-if="activeTab === 'overview'" class="cd__grid">
        ...
        <VipCard>
          ...
          <div v-if="lastTest" class="cd__test" :class="lastTest.ok ? 'is-ok' : 'is-fail'">
            ...
            <div>
              <div class="cd__test-title">{{ lastTest.ok ? 'Healthy' : 'Failed' }} · {{ formatDuration(lastTest.latencyMs) }}</div>
      ...
      <VipCard v-else-if="activeTab === 'configuration'">
        ...
        <dl class="cd__facts cd__facts--wide">
          <div class="cd__fact"><dt>Connector type</dt><dd>{{ CONNECTOR_LABEL[connection.connector] }}</dd></div>
          <div class="cd__fact"><dt>Endpoint</dt><dd class="is-mono">{{ connection.host ?? '—' }}</dd></div>
          <div class="cd__fact"><dt>Owner</dt><dd>{{ connection.owner }}</dd></div>
          <div class="cd__fact"><dt>SSL / TLS</dt><dd>Enabled (verify-full)</dd></div>
          <div class="cd__fact"><dt>Pool size</dt><dd>10 connections</dd></div>
          <div class="cd__fact"><dt>Read replica</dt><dd>Preferred</dd></div>
        </dl>
      </VipCard>
      ...
      <VipCard v-else-if="activeTab === 'schema'" :padded="false">
        <VipTable :columns="schemaColumns" :rows="schemaRows" :row-key="(r) => r.table" density="compact">
          <template #cell-table="{ row }"><span class="cd__mono">{{ row.table }}</span></template>
          <template #cell-type="{ row }">
            <VipBadge :tone="row.type === 'view' ? 'info' : 'neutral'" variant="soft" size="sm">{{ row.type }}</VipBadge>
          </template>
      ...
      <section v-else-if="activeTab === 'health'" class="cd__grid">
        <VipCard>
          ...
          <div class="cd__health-status">
            <VipBadge :tone="STATUS_TONE[connection.status]" variant="soft" size="md">{{ STATUS_LABEL[connection.status] }}</VipBadge>
            <span class="cd__muted">Last checked {{ relativeTime(connection.lastTested) }}</span>
          </div>
          <div class="cd__stats">
            <div class="cd__stat"><span class="cd__stat-value">99.94%</span><span class="cd__stat-label">Uptime (30d)</span></div>
            <div class="cd__stat"><span class="cd__stat-value">{{ formatDuration(66) }}</span><span class="cd__stat-label">Avg latency</span></div>
            <div class="cd__stat"><span class="cd__stat-value">1.2k</span><span class="cd__stat-label">Syncs (30d)</span></div>
          </div>
        </VipCard>
        <VipCard>
        ...
        <ul class="cd__feed">
          <li v-for="a in activity" :key="a.id" class="cd__feed-item">
            ...
            <div class="cd__feed-body">
              <span class="cd__feed-text"><strong>{{ a.actor }}</strong> {{ a.action }}</span>
      ...
      <VipCard v-else :padded="false">
        <VipTable :columns="auditColumns" :rows="audit" :row-key="(r) => r.id" density="compact">
          <template #cell-event="{ row }"><span class="cd__mono">{{ row.event }}</span></template>
          <template #cell-ip="{ row }"><span class="cd__mono">{{ row.ip }}</span></template>
        </VipTable>
      </VipCard>
    </template>
  </div>
...
<style scoped>
.cd { max-width: 1200px; margin: 0 auto; }
.cd__loading { display: flex; justify-content: center; padding: var(--vip-sp-12); }
.cd__connector { display: inline-flex; align-items: center; gap: var(--vip-sp-3); font-size: var(--vip-fs-sm); color: var(--vip-text-muted); }

.cd__grid { display: grid; grid-template-columns: 1fr 1fr; gap: var(--vip-sp-6); align-items: start; }
.cd__card-title { font-size: var(--vip-fs-md); font-weight: var(--vip-fw-semibold); margin-bottom: var(--vip-sp-5); }

.cd__facts { display: flex; flex-direction: column; gap: 0; }
.cd__facts--wide { display: grid; grid-template-columns: 1fr 1fr; gap: 0 var(--vip-sp-8); }
.cd__fact { display: flex; justify-content: space-between; gap: var(--vip-sp-6); padding: var(--vip-sp-4) 0; border-bottom: 1px solid var(--vip-border-subtle); }
.cd__fact dt { font-size: var(--vip-fs-sm); color: var(--vip-text-muted); }
.cd__fact dd { font-size: var(--vip-fs-md); color: var(--vip-text-primary); font-weight: var(--vip-fw-medium); text-align: right; }
.cd__fact dd.is-mono { font-family: var(--vip-font-mono); font-weight: var(--vip-fw-regular); font-size: var(--vip-fs-sm); }

.cd__test { display: flex; gap: var(--vip-sp-5); padding: var(--vip-sp-5); border-radius: var(--vip-radius-md); }
.cd__test.is-ok { background: var(--vip-success-soft); color: var(--vip-success-text); }
.cd__test.is-fail { background: var(--vip-danger-soft); color: var(--vip-danger-text); }
.cd__test-title { font-size: var(--vip-fs-md); font-weight: var(--vip-fw-semibold); color: var(--vip-text-primary); }
.cd__test-msg { font-size: var(--vip-fs-sm); color: var(--vip-text-secondary); margin-top: var(--vip-sp-2); }
.cd__muted { font-size: var(--vip-fs-sm); color: var(--vip-text-muted); }
.cd__mono { font-family: var(--vip-font-mono); font-size: var(--vip-fs-sm); color: var(--vip-text-secondary); }

.cd__health-status { display: flex; align-items: center; gap: var(--vip-sp-5); margin-bottom: var(--vip-sp-6); }
.cd__stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: var(--vip-sp-5); }
.cd__stat { display: flex; flex-direction: column; gap: var(--vip-sp-2); padding: var(--vip-sp-5); background: var(--vip-surface-2); border-radius: var(--vip-radius-md); }
.cd__stat-value { font-size: var(--vip-fs-xl); font-weight: var(--vip-fw-bold); font-variant-numeric: tabular-nums; }
.cd__stat-label { font-size: var(--vip-fs-xs); color: var(--vip-text-muted); }

.cd__spark { display: flex; align-items: flex-end; gap: var(--vip-sp-3); height: 96px; margin-bottom: var(--vip-sp-5); }
.cd__spark-bar { flex: 1; background: var(--vip-brand-500); border-radius: var(--vip-radius-xs) var(--vip-radius-xs) 0 0; min-height: 4px; }

.cd__preview-head { display: flex; align-items: center; justify-content: space-between; padding: var(--vip-sp-5) var(--vip-sp-6); border-bottom: 1px solid var(--vip-border-subtle); }

.cd__deps { list-style: none; margin: var(--vip-sp-5) 0 0; padding: 0; display: flex; flex-direction: column; gap: var(--vip-sp-3); }
.cd__dep { display: flex; align-items: center; gap: var(--vip-sp-5); padding: var(--vip-sp-4) var(--vip-sp-5); border: 1px solid var(--vip-border-subtle); border-radius: var(--vip-radius-md); }
.cd__dep-icon { color: var(--vip-text-muted); }
.cd__dep-name { flex: 1; font-size: var(--vip-fs-md); color: var(--vip-text-primary); font-weight: var(--vip-fw-medium); }

.cd__feed { list-style: none; margin: 0; padding: 0; }
.cd__feed-item { display: flex; gap: var(--vip-sp-4); padding: var(--vip-sp-4) 0; }
.cd__feed-dot { width: 26px; height: 26px; flex: none; display: inline-flex; align-items: center; justify-content: center; border-radius: 50%; background: var(--vip-surface-3); color: var(--vip-text-muted); }
.cd__feed-body { display: flex; flex-direction: column; gap: 2px; }
.cd__feed-text { font-size: var(--vip-fs-sm); color: var(--vip-text-secondary); }
.cd__feed-time { font-size: var(--vip-fs-2xs); color: var(--vip-text-disabled); }

@media (max-width: 860px) { .cd__grid, .cd__facts--wide, .cd__stats { grid-template-columns: 1fr; } }
</style>
