...
const wellDefs = computed<WellDef[]>(() => {
  ...
  if (t === 'scatter')
    return [
      { key: 'category', label: 'Details', kind: 'dim' },
      { key: 'values', label: 'X / Y / Size', kind: 'measure' },
    ]
  if (t === 'table' || t === 'pivot')
    return [
      { key: 'category', label: 'Rows', kind: 'dim' },
      { key: 'values', label: 'Values', kind: 'measure' },
    ]
...
function setAgg(fieldId: string, agg: Aggregation) {
  patchWells({
    values: (w.value?.wells.values ?? []).map((v) => (v.fieldId === fieldId ? { ...v, aggregation: agg } : v)),
  })
}
...
const vizTypeOptions = WIDGET_CATALOG.filter((c) => c.group !== 'Content' && c.group !== 'Filter').map((c) => ({
  value: c.type,
  label: c.label,
}))
</script>
  ...
  <aside class="winsp">
    <div v-if="!w" class="winsp__empty">
      <VipEmptyState
        ...
        description="Select a widget to configure its data, formatting and interactions."
      />
    </div>
    <template v-else>
      ...
      <div class="winsp__body">
        ...
        <template v-if="tab === 'build'">
          <div class="winsp__section">
            ...
            <VipSelect
              ...
              size="sm"
              @update:model-value="changeType($event as WidgetType)"
            />
          </div>
          ...
          <div
            v-for="well in wellDefs"
            :key="well.key"
            class="winsp__well"
            @dragover.prevent
            @drop="well.kind === 'dim' ? onDropDim(well.key as DimWellKey, $event) : onDropMeasure($event)"
          >
            <div class="winsp__well-head">
              ...
              >
                <template #trigger
                  ><button class="winsp__add"><VipIcon name="plus" :size="12" /></button
                ></template>
              </VipMenu>
              <VipMenu
                v-else
            ...
            <div class="winsp__chips">
              <template v-if="well.kind === 'dim'">
                <div v-for="fid in dimValues(well.key as DimWellKey)" :key="fid" class="winsp__chip">
                  ...
                  <button class="winsp__chip-x" @click="removeDim(well.key as DimWellKey, fid)">
              ...
              <template v-else>
                <div v-for="v in w.wells.values ?? []" :key="v.fieldId" class="winsp__chip is-measure">
                  ...
                  <select
                    class="winsp__agg"
                    :value="v.aggregation"
                    @change="setAgg(v.fieldId, ($event.target as HTMLSelectElement).value as Aggregation)"
                  >
                    <option v-for="o in aggOptions" :key="o.value" :value="o.value">{{ o.label }}</option>
                  </select>
                  <button class="winsp__chip-x" @click="removeMeasure(v.fieldId)">
                    <VipIcon name="close" :size="11" />
                  </button>
                </div>
              </template>
              <div
                v-if="
                  (well.kind === 'dim' ? dimValues(well.key as DimWellKey).length : (w.wells.values ?? []).length) === 0
                "
                class="winsp__well-empty"
              >
          ...
          <div v-if="w.type === 'text' || w.type === 'rich-text' || w.type === 'image'" class="winsp__section">
            ...
            <VipInput
              :model-value="w.content ?? ''"
              size="sm"
              @update:model-value="editor.patchWidget(w!.id, { content: String($event) })"
            />
          </div>
        </template>
        ...
        <template v-else-if="tab === 'format'">
          <div class="winsp__section">
            <VipInput
              label="Title"
              :model-value="w.format.title ?? ''"
              size="sm"
              @update:model-value="patchFormat({ title: String($event) })"
            />
          </div>
          <div class="winsp__section">
            <VipInput
              label="Subtitle"
              :model-value="w.format.subtitle ?? ''"
              size="sm"
              @update:model-value="patchFormat({ subtitle: String($event) })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.format.showTitle"
              label="Show title"
              @update:model-value="patchFormat({ showTitle: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.format.showLegend"
              label="Show legend"
              @update:model-value="patchFormat({ showLegend: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.format.showDataLabels"
              label="Data labels"
              @update:model-value="patchFormat({ showDataLabels: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.format.showGridlines"
              label="Gridlines"
              @update:model-value="patchFormat({ showGridlines: $event })"
            />
          </div>
          <div class="winsp__section">
            <VipSelect
              label="Number format"
              :model-value="w.format.numberStyle"
              size="sm"
              :options="[
              ...
              @update:model-value="patchFormat({ numberStyle: $event as 'plain' })"
            />
          </div>
          <div class="winsp__section">
            <VipInput
              ...
              type="number"
              :model-value="w.format.decimals"
              size="sm"
              @update:model-value="patchFormat({ decimals: Number($event) })"
            />
          </div>
          <div class="winsp__section">
            <VipSelect
              label="Color scheme"
              :model-value="w.format.colorScheme ?? 'default'"
              size="sm"
              :options="[
                { value: 'default', label: 'VIP Default' },
                { value: 'cool', label: 'Cool' },
                { value: 'warm', label: 'Warm' },
                { value: 'status', label: 'Status' },
              ]"
              @update:model-value="patchFormat({ colorScheme: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.format.border"
              label="Border"
              @update:model-value="patchFormat({ border: $event })"
            />
          </div>
        </template>
        ...
        <template v-else-if="tab === 'interactions'">
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.interactions.crossFilter"
              label="Cross-filter other visuals"
              @update:model-value="patchInteractions({ crossFilter: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.interactions.drillDown"
              label="Enable drill-down"
              @update:model-value="patchInteractions({ drillDown: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.interactions.tooltip"
              label="Show tooltips"
              @update:model-value="patchInteractions({ tooltip: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.interactions.exportable"
              label="Allow export"
              @update:model-value="patchInteractions({ exportable: $event })"
            />
          </div>
          <p class="winsp__hint">
            Drill-through to another page is a foundation; target selection connects to the backend page registry.
          </p>
        </template>
        ...
        <template v-else>
          <div class="winsp__section">
            <VipInput
              label="Widget name"
              :model-value="w.general.name"
              size="sm"
              @update:model-value="patchGeneral({ name: String($event) })"
            />
          </div>
          <div class="winsp__section">
            <VipInput
              label="Description"
              :model-value="w.general.description ?? ''"
              size="sm"
              @update:model-value="patchGeneral({ description: String($event) })"
            />
          </div>
          <div class="winsp__section">
            <VipInput
              label="Accessibility label"
              :model-value="w.general.ariaLabel ?? ''"
              size="sm"
              help="Announced to screen readers."
              @update:model-value="patchGeneral({ ariaLabel: String($event) })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.general.visible"
              label="Visible"
              @update:model-value="patchGeneral({ visible: $event })"
            />
          </div>
          <div class="winsp__row">
            <VipSwitch
              :model-value="w.general.locked"
              label="Lock position & size"
              @update:model-value="patchGeneral({ locked: $event })"
            />
          </div>
...
.winsp__tabs {
  padding: var(--vip-sp-5) var(--vip-sp-5) 0;
}
.winsp__tabs :deep(.vip-seg) {
  width: 100%;
}
.winsp__tabs :deep(.vip-seg__btn) {
  flex: 1;
