...
<template>
  <div class="wrap">
    <VipPageHeader
      ...
      description="Shared, governed definitions of business terms with owners, stewards and approval state."
    >
      <template #actions>
    ...
    <div class="layout">
      <aside class="list">
        ...
        <div v-if="isLoading" class="list__body">
          <div v-for="n in 6" :key="n" class="list__sk">
            <VipSkeleton width="70%" /><VipSkeleton width="40%" height="10px" style="margin-top: 6px" />
          </div>
        </div>
      ...
      <section class="detail">
        <template v-if="selected">
          ...
          <div class="detail__grid">
            <div class="kv">
              <span class="kv__k"><VipIcon name="users" :size="13" /> Owner</span
              ><span class="kv__v">{{ selected.owner }}</span>
            </div>
            <div class="kv">
              <span class="kv__k"><VipIcon name="shield" :size="13" /> Steward</span
              ><span class="kv__v">{{ selected.steward }}</span>
            </div>
          </div>
          ...
          <div v-if="selected.relatedTerms.length" class="block">
            ...
            <div class="chips">
              <span v-for="r in selected.relatedTerms" :key="r" class="chip chip--link"
                ><VipIcon name="link" :size="11" /> {{ r }}</span
              >
            </div>
          </div>
          ...
          <div v-if="selected.linkedDatasets.length" class="block">
            ...
            <div class="chips">
              <span v-for="d in selected.linkedDatasets" :key="d" class="chip chip--data"
                ><VipIcon name="database" :size="11" /> {{ d }}</span
              >
            </div>
          </div>
        </template>
        <VipEmptyState
          ...
          icon="book"
          title="Select a term"
          description="Choose a glossary term to view its definition and governance."
        />
      </section>
    </div>
    ...
    <VipDialog
      ...
      description="Terms are created as drafts and require a steward's approval."
    ...
    >
      <div class="form">
        <VipInput
          ...
          required
          :error="draft.term ? termError : ''"
        />
        <VipTextarea
          v-model="draft.definition"
          label="Definition"
          :rows="3"
          placeholder="A precise, unambiguous definition."
          required
        />
        <div class="row2">
        ...
        <VipInput
          ...
          help="Alternate names people use for this term."
        />
      </div>
      <template #footer>
