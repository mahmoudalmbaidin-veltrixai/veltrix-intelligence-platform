...
<template>
  <div>
    <VipPageHeader
      ...
      description="Execution history across all automations, including dead-letter failures."
    />
    <VipTable
      :columns="columns"
      :rows="data ?? []"
    ...
    <VipDrawer :open="!!selected" :title="selected?.automation" :width="460" @close="selected = null">
      <template v-if="selected">
        <div class="ar-status">
          <VipBadge :tone="tone(selected.status)">{{ selected.status }}</VipBadge
          ><span>{{ relativeTime(selected.startedAt) }} · {{ formatDuration(selected.durationMs) }}</span>
        </div>
        ...
        <div class="ar-actions">
          <VipButton
            v-if="selected.status === 'failed' || selected.status === 'dead-letter'"
            variant="secondary"
            size="sm"
            icon="refresh"
            @click="act('Retry run')"
          ...
          <VipButton
            ...
            size="sm"
            icon="close"
            @click="act('Cancel run')"
            >Cancel</VipButton
          >
          <VipButton
            v-if="selected.status === 'waiting-approval'"
            variant="primary"
            size="sm"
            icon="check"
            @click="act('Resume after approval')"
            >Resume</VipButton
          >
          <VipButton variant="tertiary" size="sm" icon="copy" @click="act('Copy diagnostics')">Diagnostics</VipButton>
        </div>
        <div v-if="selected.status === 'dead-letter'" class="ar-dead">
          <VipIcon name="warning" :size="14" /> Moved to dead-letter after exhausting retries. Duplicate-action
          protection prevented re-execution of completed steps.
        </div>
      </template>
    </VipDrawer>
  </div>
