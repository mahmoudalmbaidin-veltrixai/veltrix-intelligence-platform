...
function toggleMeasure(f: SemanticField) {
  ...
  const next = exists
    ? cur.filter((v) => v.fieldId !== f.id)
    : [...cur, { fieldId: f.id, aggregation: (f.defaultAggregation ?? 'sum') as Aggregation }]
  widget.value = { ...widget.value, wells: { ...widget.value.wells, values: next } }
}

const showData = ref(false)
function save() {
  ui.pushToast({ kind: 'success', title: 'Saved as insight', message: 'Available in Insights → Saved.' })
}
function pin() {
  ui.pushToast({ kind: 'success', title: 'Pinned to dashboard' })
}
function exportData() {
  ui.pushToast({ kind: 'info', title: 'Export', message: 'CSV export is a backend dependency.' })
}
  ...
  <div class="explore">
    <header class="explore__toolbar">
      ...
      <div class="explore__tb-right">
        <VipButton variant="ghost" size="sm" :icon="'table'" :active="showData" @click="showData = !showData"
          >Data</VipButton
        >
      ...
      <aside class="explore__fields">
        ...
        <div class="explore__field-group">
          ...
          <button
            v-for="f in dims"
            :key="f.id"
            class="explore__field"
            :class="{ 'is-active': isActiveDim(f.id) }"
            @click="toggleDim(f)"
          >
        ...
        <div class="explore__field-group">
          ...
          <button
            v-for="f in measures"
            :key="f.id"
            class="explore__field is-measure"
            :class="{ 'is-active': isActiveMeasure(f.id) }"
            @click="toggleMeasure(f)"
          >
            <VipIcon :name="roleIcon(f.role)" :size="13" /><span>{{ f.label }}</span>
...
.explore__field:hover {
  background: var(--vip-surface-hover);
}
.explore__field.is-active {
  background: var(--vip-brand-soft);
  color: var(--vip-brand-text);
}
.explore__field.is-measure.is-active {
  color: var(--vip-brand-text);
}
.explore__check {
  color: var(--vip-brand-text);
...
.explore__ctype:hover {
  border-color: var(--vip-border-strong);
  color: var(--vip-text-primary);
}
.explore__ctype.is-active {
  background: var(--vip-brand-soft);
