...
const columns: Column<WorkspaceRow>[] = [
  { key: 'name', label: 'Workspace' }, { key: 'members', label: 'Members', align: 'right' },
  { key: 'archived', label: 'State' }, { key: 'createdAt', label: 'Created' }, { key: 'actions', label: '', align: 'right' },
]
function menuFor(w: WorkspaceRow) {
  return [
    { key: 'edit', label: 'Edit', icon: 'settings' },
    w.archived ? { key: 'restore', label: 'Restore', icon: 'refresh' } : { key: 'archive', label: 'Archive', icon: 'folder' },
    { key: 'delete', label: 'Delete', icon: 'trash', danger: true },
  ]
}
function onMenu(w: WorkspaceRow, key: string) {
  if (key === 'archive') { w.archived = true; ui.pushToast({ kind: 'warning', title: 'Workspace archived', message: w.name }) }
  else if (key === 'restore') { w.archived = false; ui.pushToast({ kind: 'success', title: 'Workspace restored', message: w.name }) }
  else ui.pushToast({ kind: 'info', title: key, message: `${w.name} — dependency checks run before ${key}.` })
}
function create() {
  rows.value.unshift({ id: `ws_${Date.now()}`, name: newName.value, members: 1, archived: false, createdAt: new Date().toISOString() })
...
<template>
  <div>
    <VipPageHeader title="Workspace Administration" description="Create, edit, archive and govern workspaces.">
      <template #actions><VipButton variant="primary" icon="plus" @click="createOpen = true">New workspace</VipButton></template>
    </VipPageHeader>
    <VipAlert tone="info" title="Workspace isolation">Resources and permissions are isolated per workspace. Archiving hides a workspace without deleting its data.</VipAlert>
    <VipTable :columns="columns" :rows="rows" :row-key="(r) => r.id" style="margin-top: 16px">
      <template #cell-archived="{ row }"><VipBadge :tone="row.archived ? 'neutral' : 'success'" size="sm">{{ row.archived ? 'archived' : 'active' }}</VipBadge></template>
