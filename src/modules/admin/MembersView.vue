...
function changeRole(m: Member, role: string) {
  m.role = role
  ui.pushToast({ kind: 'success', title: 'Role updated', message: `${m.name} → ${role}` })
}
function invite() {
...
<template>
  <div>
    <VipPageHeader title="Members & Roles" description="Manage organization membership and role assignments.">
      <template #actions
        ><VipButton variant="primary" icon="plus" @click="inviteOpen = true">Invite member</VipButton></template
      >
    </VipPageHeader>
    <VipTable :columns="columns" :rows="members" :row-key="(r) => r.id">
      ...
      <template #cell-actions="{ row }"
        ><VipButton
          ...
          size="xs"
          icon="dotsV"
          @click="ui.pushToast({ kind: 'info', title: 'Member actions', message: row.name })"
      /></template>
    </VipTable>
    ...
    <VipDialog :open="inviteOpen" title="Invite member" @close="inviteOpen = false">
      ...
      <div style="margin-top: 12px">
        <VipSelect v-model="inviteRole" label="Role" :options="ASSIGNABLE_ROLES.map((r) => ({ value: r, label: r }))" />
      </div>
      <template #footer>
