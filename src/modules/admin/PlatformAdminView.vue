...
const rows = computed(() =>
  filter.value === 'all' ? (data.value ?? []) : (data.value ?? []).filter((o) => o.status === filter.value),
)
      ...
      <template v-if="selected">
        ...
        <VipButton
          ...
          @click="
            ui.pushToast({
              kind: 'info',
              title: 'Impersonation',
              message: 'Support impersonation is a governed backend capability (audited).',
            })
          "
          >Request support access</VipButton
        >
        ...
        <div class="pa-life">
          <VipButton
            variant="tertiary"
            size="sm"
            @click="ui.pushToast({ kind: 'info', title: 'Suspend tenant', message: 'Requires backend confirmation.' })"
            >Suspend</VipButton
          >
          <VipButton
            variant="danger"
            ...
            @click="
              ui.pushToast({
                ...
                message: 'Pending-deletion workflow is backend-gated.',
              })
            "
...
.pa-section {
  ...
  color: var(--vip-text-muted);
  margin: var(--vip-sp-7) 0 var(--vip-sp-4);
}
.pa-life {
  display: flex;
  gap: var(--vip-sp-3);
