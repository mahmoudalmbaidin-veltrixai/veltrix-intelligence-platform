...
const kindIcon: Record<string, string> = {
  ...
  variance: 'activity',
  anomaly: 'warning',
  'top-increase': 'trendUp',
  'top-decrease': 'trendDown',
...
const tone = computed(() =>
  props.insight.sentiment === 'positive' ? 'success' : props.insight.sentiment === 'negative' ? 'danger' : 'info',
)
const model = computed(() => MODELS.find((m) => m.id === props.insight.modelId)?.label ?? props.insight.modelId)
const sparkColor = computed(() =>
  props.insight.sentiment === 'positive'
    ? 'var(--vip-success)'
    : props.insight.sentiment === 'negative'
      ? 'var(--vip-danger)'
      : 'var(--vip-info)',
)
</script>
...
<template>
  <VipCard class="ic" :padded="false">
    <div class="ic__head">
      <span class="ic__kind" :class="`is-${tone}`"
        ><VipIcon :name="kindIcon[insight.kind]" :size="14" />{{ kindLabel[insight.kind] }}</span
      >
    ...
    <div class="ic__body">
      ...
      <div class="ic__metric-row">
        <div>
          <div class="ic__metric">
            {{ formatNumber(insight.metricValue, { style: insight.metricFormat, currency: 'USD' }) }}
          </div>
          <div class="ic__compare">
            <VipBadge :tone="insight.changePct >= 0 ? 'success' : 'danger'" size="sm">{{
              formatPct(insight.changePct)
            }}</VipBadge>
            {{ insight.comparisonLabel }}
          </div>
        </div>
        <div v-if="insight.series" class="ic__spark">
          <Sparkline :values="insight.series.map((s) => s.value)" :color="sparkColor" area />
        </div>
      </div>
      ...
      <div class="ic__meta">
        <span title="Confidence"
          ><VipIcon name="gauge" :size="12" /> {{ Math.round(insight.confidence * 100) }}% confidence</span
        >
    ...
    <footer class="ic__foot">
      ...
      <VipButton variant="ghost" size="xs" :icon="'pin'" :active="insight.pinned" @click="emit('pin', insight)">{{
        insight.pinned ? 'Pinned' : 'Pin'
      }}</VipButton>
      <VipButton variant="ghost" size="xs" :icon="'star'" :active="insight.saved" @click="emit('save', insight)">{{
