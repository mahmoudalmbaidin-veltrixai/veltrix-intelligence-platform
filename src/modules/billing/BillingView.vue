...
function pct(u: UsageLine) {
  return Math.min(100, Math.round((u.used / u.limit) * 100))
}
function usageTone(u: UsageLine) {
  const p = pct(u)
  return p >= 100 ? 'danger' : p >= 80 ? 'warning' : 'info'
}
function planAction(name: string) {
  ui.pushToast({
    kind: 'info',
    title: `${name}`,
    message: 'Plan changes and payment are backend-gated — no charge is made here.',
  })
}
    ...
    <div class="bill">
        ...
        <div class="bill-grid">
          ...
          <VipCard>
            ...
            <div v-for="u in (usage ?? []).slice(0, 3)" :key="u.label" class="bill-usage">
              <div class="bill-usage-head">
                <span>{{ u.label }}</span
                ><span
                  >{{ formatNumber(u.used, { style: 'compact' }) }} /
                  {{ formatNumber(u.limit, { style: 'compact' }) }}</span
                >
              </div>
              <div class="bill-bar">
                <div class="bill-bar-fill" :class="`is-${usageTone(u)}`" :style="{ width: `${pct(u)}%` }" />
              </div>
            </div>
          </VipCard>
        </div>
        <VipAlert tone="info" title="Billing is backend-managed"
          >Plan changes, proration and payments are processed by the billing service. This UI never charges a
          card.</VipAlert
        >
      </template>
      ...
      <template v-else-if="tab === 'plans'">
        <div class="bill-plans">
          <VipCard v-for="p in plans" :key="p.key" class="bill-planc" :class="{ 'is-current': p.current }">
            <div class="bill-planc-name">
              {{ p.name }}<VipBadge v-if="p.current" tone="brand" size="sm">Current</VipBadge>
            </div>
            <div class="bill-planc-price">
              {{ p.price === 0 ? 'Free' : `$${formatNumber(p.price)}` }}<span v-if="p.price">/mo</span>
            </div>
            <ul class="bill-features">
              <li v-for="f in p.features" :key="f"><VipIcon name="check" :size="13" />{{ f }}</li>
            </ul>
            <VipButton
              v-if="!p.current"
              :variant="p.price > 4800 ? 'primary' : 'secondary'"
              block
              @click="planAction(p.price > 4800 ? 'Upgrade' : 'Change plan')"
              >{{ p.price > 4800 ? 'Upgrade' : 'Switch' }}</VipButton
            >
      ...
      <template v-else-if="tab === 'usage'">
        <div class="bill-usage-grid">
          <VipCard v-for="u in usage" :key="u.label">
            <div class="bill-usage-head">
              <span>{{ u.label }}</span
              ><VipBadge :tone="usageTone(u)" size="sm">{{ pct(u) }}%</VipBadge>
            </div>
            <div class="bill-bar">
              <div class="bill-bar-fill" :class="`is-${usageTone(u)}`" :style="{ width: `${pct(u)}%` }" />
            </div>
      ...
      <template v-else-if="tab === 'invoices'">
        <VipTable :columns="invColumns" :rows="invoices ?? []" :row-key="(r) => r.id">
          ...
          <template #cell-amount="{ row }">{{
            formatNumber(row.amount, { style: 'currency', currency: 'USD', decimals: 0 })
          }}</template>
          ...
          <template #cell-actions="{ row }"
            ><VipButton variant="ghost" size="xs" icon="download" @click="planAction(`Download ${row.number}`)"
              >PDF</VipButton
            ></template
          >
        </VipTable>
      ...
      <template v-else>
        <VipCard class="bill-pay">
          ...
          <div class="bill-card">
            <VipIcon name="card" :size="20" /><span>{{ pm?.brand }} ending {{ pm?.last4 }}</span
            ><span class="bill-exp">exp {{ pm?.expiry }}</span>
          </div>
          ...
          <p class="bill-note">
            Billing contact, VAT / tax ID and address are entered directly by the account owner — this UI does not store
            payment data.
          </p>
        </VipCard>
      </template>
...
.bill-bar-fill.is-info {
  background: var(--vip-info);
}
.bill-bar-fill.is-warning {
  background: var(--vip-warning);
}
.bill-bar-fill.is-danger {
...
.bill-features li :deep(.vip-icon) {
