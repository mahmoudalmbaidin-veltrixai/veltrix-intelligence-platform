...
<template>
  <div>
    <VipPageHeader title="Pipeline runs" description="Execution history, status and diagnostics.">
      <template #actions>
        <VipButton variant="tertiary" icon="chevronLeft" @click="router.push(`/pipelines/${id}`)">Open in studio</VipButton>
      </template>
    </VipPageHeader>
    ...
    <VipTable :columns="columns" :rows="runs" :row-key="(r) => r.id" clickable @row-click="(r) => (selected = r)">
      ...
      <template #cell-correlationId="{ row }"><span class="rn-cid">{{ row.correlationId }}</span></template>
    </VipTable>

    <VipDrawer :open="!!selected" :title="`Run ${selected?.correlationId}`" :width="480" @close="selected = null">
      <template v-if="selected">
        <div class="rn-facts">
          <div class="rn-fact"><span>Status</span><VipBadge :tone="tone(selected.status)" size="sm">{{ selected.status }}</VipBadge></div>
          ...
          <div class="rn-fact"><span>Correlation ID</span><span class="rn-cid">{{ selected.correlationId }}</span></div>
        </div>
        <div class="rn-actions">
          <VipButton v-if="selected.status === 'failed'" variant="secondary" size="sm" icon="refresh">Retry run</VipButton>
...
<style scoped>
.rn-cid { font-family: var(--vip-font-mono); font-size: var(--vip-fs-xs); color: var(--vip-text-muted); }
.rn-facts { display: flex; flex-direction: column; gap: var(--vip-sp-4); }
.rn-fact { display: flex; align-items: center; justify-content: space-between; padding: var(--vip-sp-4) 0; border-bottom: 1px solid var(--vip-border-subtle); font-size: var(--vip-fs-sm); }
.rn-fact span:first-child { color: var(--vip-text-muted); }
.rn-actions { display: flex; gap: var(--vip-sp-4); margin-top: var(--vip-sp-6); }
</style>
