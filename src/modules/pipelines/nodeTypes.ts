  ...
  'source-database': {
    ...
    config: [
      ...
      },
      {
        ...
        visibleWhen: { key: 'mode', equals: 'table' },
      },
      {
        key: 'query',
        label: 'SQL query',
        ...
        visibleWhen: { key: 'mode', equals: 'query' },
      },
      {
        key: 'incremental',
        label: 'Incremental load',
        type: 'boolean',
        help: 'Only read rows newer than the last watermark.',
      },
    ],
  },
  'source-file': {
    ...
    config: [
      {
        ...
        required: true,
        options: [
          { value: 'cn_s3_lake', label: 'Data Lake (S3)' },
  ...
  'source-rest': {
    ...
    config: [
      ...
      },
      {
        ...
        label: 'Headers',
        type: 'keyvalue',
        help: 'Authorization is resolved from the connection secret at run time.',
      },
    ...
    config: [
      {
        ...
        options: [
          ...
          { value: 'left', label: 'Left outer' },
          { value: 'right', label: 'Right outer' },
  ...
  formula: {
    ...
    config: [
      { key: 'columnName', label: 'New column name', type: 'text', required: true, placeholder: 'margin_pct' },
      {
        ...
        type: 'formula',
        required: true,
        placeholder: 'IF([revenue] > 0, ROUND([profit] / [revenue], 2), 0)',
      },
    ],
  },
  'type-convert': {
    ...
    config: [
      {
        ...
        type: 'keyvalue',
        required: true,
        help: 'column → type (string/number/date/boolean)',
      },
    ],
  },
  deduplicate: {
    ...
    outputs: OUT,
    config: [
      { key: 'keys', label: 'Key columns', type: 'text', placeholder: 'order_id', help: 'Empty = full-row dedup.' },
    ],
  },
  'null-handling': {
    kind: 'null-handling',
    ...
    config: [
      {
        ...
        language: 'sql',
        required: true,
        placeholder: 'SELECT region, SUM(revenue) AS revenue FROM input GROUP BY 1',
      },
    ],
  },
  'python-transform': {
    kind: 'python-transform',
    ...
    config: [
      {
        ...
        type: 'select',
        required: true,
        options: [{ value: 'cn_s3_lake', label: 'Data Lake (S3)' }],
      },
      {
        key: 'path',
        ...
        type: 'text',
        required: true,
        placeholder: 's3://lake/exports/orders.parquet',
      },
      {
        key: 'format',
        label: 'Format',
