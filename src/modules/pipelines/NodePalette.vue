...
const groups = computed(() => {
  ...
  return PALETTE_GROUPS.map((g) => ({
    ...
    kinds: g.kinds.filter(
      (k) => !q || NODE_TYPES[k].label.toLowerCase().includes(q) || NODE_TYPES[k].description.toLowerCase().includes(q),
    ),
...
.palette__node:active {
  cursor: grabbing;
}
.palette__node.is-source {
  border-left-color: var(--vip-viz-2, #22c1a6);
}
.palette__node.is-transform {
  border-left-color: var(--vip-brand-500);
}
.palette__node.is-output {
