...
<template>
  <div
    ...
    :class="[
      `is-${spec.category}`,
      { 'is-selected': selected, 'is-error': errorCount, [`exec-${execStatus}`]: execStatus && execStatus !== 'idle' },
    ]"
  ...
  >
    ...
    <div
      ...
      class="pnode__badge"
      :class="errorCount ? 'is-error' : 'is-warn'"
...
.pnode.is-output {
  border-left-color: var(--vip-viz-3, #f2a93b);
}
.pnode.is-selected {
  border-color: var(--vip-brand-500);
  box-shadow:
    0 0 0 2px var(--vip-brand-soft),
    var(--vip-shadow-md);
}
.pnode.is-error {
...
.pnode.exec-running {
  box-shadow:
    0 0 0 2px var(--vip-info-soft),
    var(--vip-shadow-md);
}
.pnode.exec-succeeded {
...
.pnode__head {
  ...
  align-items: center;
  gap: var(--vip-sp-3);
  padding: var(--vip-sp-4) var(--vip-sp-5) var(--vip-sp-2);
}
.pnode__icon {
  width: 22px;
  height: 22px;
...
.pnode__port.is-out {
  border-color: var(--vip-brand-400);
}
...
.pnode__badge.is-error {
  background: var(--vip-danger);
}
.pnode__badge.is-warn {
...
.pnode__spin {
  width: 13px;
  height: 13px;
  border: 2px solid var(--vip-info);
  border-right-color: transparent;
  border-radius: 50%;
  animation: pn-spin 0.7s linear infinite;
}
.pnode__exec-icon.is-ok {
  color: var(--vip-success-text);
}
.pnode__exec-icon.is-fail {
