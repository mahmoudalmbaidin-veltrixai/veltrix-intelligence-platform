...
### High
**H1 — Automatic recurring dashboard delivery never executes. — RESOLVED in Phase B9.1A** (recurring scheduler with cron/timezone support, `FOR UPDATE SKIP LOCKED` due-claiming, duplicate prevention, retry, pause/resume, missed-slot catch-up, audit + metrics; runs as a tick inside the existing job worker. See `VIP_PHASE_B9_1A_DASHBOARD_DELIVERY_REPORT.md`.)
