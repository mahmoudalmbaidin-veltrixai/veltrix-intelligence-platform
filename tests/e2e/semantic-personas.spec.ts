...
test('semantic personas render from backend-resolved effective access', async ({ page }) => {
  ...
  const models = adminList.body as Array<{ id: string }>
  expect(Array.isArray(models) && models.length > 0).toBe(true)
  const modelId = models[0].id

  const adminDetail = await apiGet(page, `/semantic-models/${modelId}`)
  ...
  const editorDetail = await apiGet(page, `/semantic-models/${modelId}`)
  ...
  const viewerDetail = await apiGet(page, `/semantic-models/${modelId}`)
  ...
  const viewerArchive = await apiArchive(page, modelId)
  ...
  const deniedDetail = await apiGet(page, `/semantic-models/${modelId}`)
  expect([403, 404]).toContain(deniedDetail.status)
  const deniedArchive = await apiArchive(page, modelId)
